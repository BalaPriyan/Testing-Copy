    ...
    async def beforeStart(self):
        ...
        if not self.isLeech:
            ...
            if config_dict['LEECH_DUMP_CHAT']:
                self.upDest = config_dict['LEECH_DUMP_CHAT']
            else:
                if config_dict['BOT_PM']:
                    self.upDest = self.message.from_user.id
                else:
                    self.upDest = self.upDest
            self.mixedLeech = IS_PREMIUM_USER and (self.userDict.get("mixed_leech") or config_dict["MIXED_LEECH"] and "mixed_leech" not in self.userDict)
            if self.upDest:
