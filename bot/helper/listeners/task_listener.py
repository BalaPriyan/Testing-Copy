...
class TaskListener(TaskConfig):
    ...
    async def onUploadComplete(self, link, files, folders, mime_type, rclonePath="", dir_id=""):
        if (self.isSuperChat and config_dict["INCOMPLETE_TASK_NOTIFIER"] and DATABASE_URL):
            await DbManager().rm_complete_task(self.message.link)
        if config_dict['SAFE_MODE']:
            sfmsg = f"<b>Name: </b><code>Safe Mode Enabled"
        else:
        ...
        if self.isLeech:
            ...
            if not files:
                ...
                if fmsg != "":
                    if config_dict['BOT_PM'] and self.message.chat.type != self.message.chat.type.PRIVATE:
                        lmpmsg = f'<b>File Has Been Sent In Private</b>'
                        button = await get_bot_pm_button()
                        if config_dict['SAFE_MODE']:
                            if self.message.from_user.id == OWNER_ID and self.message.chat.type.PRIVATE:
                                await sendMessage(self.message, sfmsg + msg + fmsg +lmpmsg,button)
                            else:
                                await sendMessage(self.message, sfmsg + msg + lmpmsg,button)
                        else:
                            await sendMessage(self.message, sfmsg + msg + fmsg +lmpmsg,button)
                else:
                    await sendMessage(self.message, msg + fmsg)
            ...
            if config_dict['BOT_PM'] and self.message.chat.type != self.message.chat.type.PRIVATE:
                ...
                await send_to_chat(chat_id=self.message.from_user.id, text=sfmsg + msg, button=button)
                pmbutton = await get_bot_pm_button()
                await sendMessage(self.message, sfmsg+msg+bmsg, pmbutton)
        if self.seed:
            if self.newDir:
